...
if(isset($_GET["delReqImageId"])){
	   ...
	   $updateImage=array(); 
	     foreach ($getGallery as $value){
               if($value == $delImageName){  
                ...
                $unlinkGalleryPath=$path.'gallery'.DS;

						if (file_exists($unlinkGalleryPath.$value)) {
					      unlink($unlinkGalleryPath.$value); 
					  } }else{
	...
	$update = "You have deleted successfully";
    $id= ID_encode($delReqImageId)
    ;
...
if(isset($_POST['submit'])){
 ...
 $gallery = $_FILES['gallery']['name'][0];
	...
	if (!empty($header_image)) {
			  ...
			  /*  $path = "../uploads/archProfileImage/header/"; // Upload directory*/
			     $path = SITE_ROOT.DS."uploads".DS."archProfileImage".DS;
                        $unlinkheaderPath=$path.'header'.DS;
			      if (file_exists($unlinkheaderPath.$result->header_image)) {                 
				      unlink($unlinkheaderPath.$result->header_image); 
				   }
			    $image_name_in_arry="";
    ...
    // Loop $_FILES to execute all files
    	if (!empty($gallery)) {
    		...
    		//remobe previous image  working
				    ...
				    $max_file_size = 1024*4200; //4000 kb / 4MB
				    /*$path =  SITE_ROOT.DS."website".DS."images".DS."slide".DS;*/ // Upload directory
				    /*$path = "../uploads/archProfileImage/gallery/"; // Upload directory*/
				     $path = SITE_ROOT.DS."uploads".DS."archProfileImage".DS;
				   ...
				   $image_name_in_arry="";
					...
					$galleryNew=$image_name_in_arry;
			    	 ...
			    	 if(!empty($galleryNew)) {
			    	 	if(!empty($result->gallery)){
			    	 		$gallery=$result->gallery.','.$galleryNew;
			    	 	}else{
			    	 	$gallery = $galleryNew;
			    	 }
			    	 }else{
					...
					$org->update_directory_arch_req($_POST);
				...
				<div class="row">
					...
					<div class="col-md-3">
						<div class="form-group">
						   ...
						   <input type="hidden" name="pre_slug" value="<?php echo $result->slug_url; ?>" >
						  </div>
					</div>
					<div class="col-md-3">
						<div class="form-group date-wrapper">
						    <label >Slug Url</label>
						    <input class="form-control"  name="slug_url" value="<?php echo $result->slug_url; ?>" >
						  </div>
					</div>
					<div class="col-md-3">
						<div class="form-group">
							<label>Organization Contact Number <sup style="color: red; ">*</sup></label>
				  ...
				  <div class="col-md-3">
					...
					<div class="col-md-3">
						<div class="form-group">
						    ...
						    <input class="form-control" placeholder="Google Map" name="mlink" value="<?php echo $result->mlink; ?>" >
						  </div>
					</div>
					<div class="col-md-2">
						<div class="form-group date-wrapper">
					...
					<div class="col-md-3">  
						<div class="form-group">
						 ...
						 <div class="form-group">
						    ...
						    <img name="header_image" style='width: 50px; height: 50px;' src="<?php echo SITE_URL; ?>uploads/archProfileImage/header/<?php echo $result->header_image; ?>">
						    <?php } ?>
						 </div>
					</div>
						  	 ...
						  	 <?php 
							    if (!empty($result->gallery)) {
								    ...
								    <?php foreach ($getGallery as $value): ?>
										<img name="gallery" style="width: 50px; height:50px;" src="<?php echo SITE_URL; ?>uploads/archProfileImage/gallery/<?php echo $value; ?>"><sup><a href="?delReqImageId=<?php echo ID_encode($result->id); ?>&filename=<?php echo $value; ?>" style="color: red;" onclick="if (! confirm('Do you want to delete ?')) { return false; }" >X</a></sup>
									<?php endforeach ?>
								<?php } ?>
						  </div>
					</div>
					...
					<div class="col-md-12">
						<div class="form-group">
		            	   ...
		            	   <textarea name="intro_text" class="remarks" id="summernote" placeholder=" write here" rows="5" ><?php echo $result->intro_text; ?></textarea>
						</div>
					</div>				
					...
					<div class="col-md-12 text-right" style="margin-top: 10px;">
						...
						<a href="directory-org-request.php" class="btn btn-warning btn-rounded">Back</a>
...
<script src="<?php echo SITE_URL; ?>assets/js/plugin/tinymce/tinymce.min.js"></script>
